...
class Predictor_threads(QThread):
    ...
    signal_kill = pyqtSignal(bool) #终止线程的信号
    ...
    def run(self):
        ...
        if self.config['predict_way'] == 'Video':
            ...
            #不是空帧或者没有停止信号时就循环读取
            while self.ret and self.keep_run:
...
class PaddleX_Serving:
    def __init__(self):
        ...
        self.mainwindows_ui.About.clicked.connect(self.open_about)
        #About窗口
        self.about = QWidget()
    ...
    def open_load(self):
        ...
        self.load_model_ui.Load_Model_tip.setStyleSheet("background-color: rgb(255, 85, 0);\n"
    ...
    def select_model_bt(self):
        ...
        if directory:
            ...
            self.select_model_ui.model_dir.setStyleSheet("background-color: rgba(71, 255, 43);\n"
                                                            "border:2px solid rgb(85, 85, 255);\n"
                                                            "border-radius:10px;\n"
                                                            "")
    ...
    def run_serving(self):
        ...
        for i in range(98):
            time.sleep(0.01)
            self.load_model_ui.progressBar.setProperty("value", count)
            count += 1
    ...
    def add_picture_path(self):
        ...
        if picture_absolute_path:
            ...
            if not relative_path.endswith('.jpg'):
                self.warning_ui.label.setText('请选择jpg图片')
                self.warning.show()
                return
    ...
    def run_predict(self):
        ...
        点击预测按钮完成预测
        '''
        if self.config['Predicter_thread_run'] == True:
            #发送停止信号
            self.Predicter_thread.signal_kill.emit(False)
            self.mainwindows_ui.Bt_Predict.setText('正在停止')
            return
        #读取图片
        self.mainwindows_ui.Bt_Predict.setText('点击停止')
        #按钮锁定
        self.mainwindows_ui.Start_serving.setEnabled(False)
        ...
        self.Predicter_thread.signal_kill.connect(self.Predicter_thread.stop_cmd)
        self.Predicter_thread.start()
        self.config['Predicter_thread_run'] = True
    ...
    def after_predicted_img(self,flag):
        self.config['Predicter_thread_run'] = False
        #组件解锁
        self.mainwindows_ui.Start_serving.setEnabled(True)
